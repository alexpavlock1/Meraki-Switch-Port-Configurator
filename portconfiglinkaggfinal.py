...
def configure_link_aggregation_choice():
    ...
    portentry = input("Please enter the port numbers that will be in the link aggregation. Multiple values can be entered separated with a comma (e.g., '45-48' or '1-3, 12'). For modules, use the format '1_MA-MOD-4X10G_1'. Module ports will need to be entered one by one separated by a comma: ")
    port_ids = parse_range_to_numbers(portentry)
    ...
    def configure_link_aggregation_Function(networkId, serial_list, port_ids):
        for serial in serial_list:
            ...
            try:
                ...
                serial = payload['switchPorts'][0]['serial']
                port_ids = ', '.join(port['portId'] for port in payload['switchPorts'])

                print(f"\033[1m\033[3m\033[32mSuccessfully\033[0m configured link aggregation on serial {serial} on ports {port_ids}.")
            except Exception as e:
                print(f"\033[1m\033[3m\033[31mError\033[0m configuring link aggregation with the following configuration: {payload}. Error: {e}")
        # If the loop completes successfully
        print("All link aggregations have been successfully configured.")
